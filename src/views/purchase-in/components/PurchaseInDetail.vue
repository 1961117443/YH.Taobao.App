          ...
          <el-col :span="24">
            ...
            <div class="postInfo-container">
              <el-row>
                ...
                <el-col :span="10">
                  <el-form-item label-width="100px" label="入库日期:" class="postInfo-container-item">
                    <el-date-picker v-model="postForm.indate" type="date" format="yyyy-MM-dd" placeholder="选择日期" />
                  </el-form-item>
                </el-col>
                <el-col :span="4">
...
import { fetchPurchaseIn , createPurchaseIn, updatePurchaseIn } from '@/api/purchase'
...
export default {
  ...
  computed: {
    ...
    displayTime: {
      ...
      get() {
        return (+new Date(this.postForm.indate))
      },
      set(val) {
        this.postForm.indate = new Date(val)
      }
    },
    totalQty() {
      let qty = 0
  ...
  created() {
    if (this.isEdit) {
      ...
      this.postForm = Object.assign(this.postForm, { billcode: new Date().getTime(), indate: new Date() })
    }
  ...
  methods: {
    ...
    fetchData(id) {
      fetchPurchaseIn(id).then(response => {
        console.log(response.data)
        this.postForm = response.data

        // just for test
        this.postForm.title += `   采购入库单号:${this.postForm.billcode}`
    ...
    draftForm() {
      if (this.postForm.billcode.length === 0 || this.postForm.indate.length === 0) {
        this.$message({
          message: '请填写必要的入库单号和入库日期',
          type: 'warning'
        })
        return
      }
      ...
      if (this.postForm.details.length > 0) {
        this.postForm.details.forEach(item => {
          ...
          if (item.totalquantity <= 0) {
            this.$message({ message: '入库数量不能小于等于0', type: 'warning' })
            detailOk = false
            return
          }
        })
